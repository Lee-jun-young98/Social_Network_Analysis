...
res <- GET(url='https://www.transfermarkt.com/laliga/transfers/wettbewerb/ES1/plus/?saison_id=2019&s_w=&leihe=1&intern=0&intern=1')
...
teamname <- c('FC Barcelona','Atlético de Madrid','Real Madrid','Getafe CF','Valencia CF','Sevilla FC','Athletic Bilbao','Real Sociedad','RCD Espanyol Barcelona','Deportivo Alavés','SD Eibar','CD Leganés','Real Betis Balompié','Villarreal CF','Levante UD','Real Valladolid CF','Celta de Vigo','CA Osasuna','Granada CF','RCD Mallorca')
...
write.csv(total, "Spain.csv", row.names=F, fileEncoding="EUC-KR")

# class(fg)
plot(fg,vertex.size=2,vertex.label=NA, edge.width=1, edge.arrow.width = 1,layout=layout.fruchterman.reingold,edge.arrow.size=0.5)
